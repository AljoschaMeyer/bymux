...
The 5th and 6th bit work the same way for all packets: if the fourth bit is not set, the packet applies to a particular stream. The id of a stream is a 64 bit integer. The header byte of such a packet is directly followed by the stream id, in the width specified by the 5th and 6th bit.  
...
Whenever a header with a 4th byte of zero is followed by an id that does not correspond to an active stream is received, the whole connection must be immediately terminated.

### Sending Data: `Credit` and `Write` Packets

Writing to a stream consumes as much credit as there are bytes written. Writing data without credit being available is forbidden. The maximum of credit for a single stream at a time is 2^64 - 1.

Credit is given to a stream with a `Credit` packet whose fourth bit is zero. The stream id is followed by the amount of credit to be added for the stream, as a big-endian int of the width indicated by the 7th and 8th header byte. If the sum of the previously available credit and the new additional credit exceeds `2^64 - 1`, the whole connection must be immediately terminated.

Data is written to a stream with a `Write` packet whose fourth bit is zero. The stream id is followed by the amount of bytes to be written, encoded as a big-endian int of the width indicated by the 7th and 8th header byte, followed by that many bytes of raw data. Writing to a stream consumes one credit per byte. Writes may not exceed the available credit. When receiving written data that hasn't been issued credit for, the whole connection must be immediately terminated.  
...
After sending a `Close` packet for a stream, no more `Write` or further `Close` packets may be sent to it by the same endpoint. After sending a `StopRead` packet for a stream, no more `Credit` or further `StopRead` packets may be sent to it. After having sent both a `Close` and a `StopRead` packet for a stream, no more `Ping` and `Pong` packets may be sent to it. Receiving any of these invalid packets means that the whole connection must be immediately terminated. The stream is however still considered alive until both a `Close` and a `StopRead` packet sent by the other endpoints have been received. At that point, the stream is fully ended and its id may be reused by further, newly created streams.
...
After having sent both a global `Close` and a global `StopRead` packet, no more global `Ping` and `Pong` packets may be sent. Receiving any of these invalid packets means that the whole connection must be immediately terminated.

### Creating Streams: Global `Credit` and `Write` Packets

The process of creating streams is also guided by backpressure, one point of global credit allows to create one stream. Global credit is given with a `Credit` packet whose fourth bit is one. Such a header is followed by the amount of credit to be added for the stream, as a big-endian int of the width indicated by the 7th and 8th header byte. If the sum of the previously available global credit and the new additional credit exceeds `2^64 - 1`, the whole connection must be immediately terminated.
...
When one endpoint creates a stream, the other endpoint has zero sending credit on that stream. The creation of the stream can be immediately followed up with a `Credit` packet, so the other endpoint can begin sending data virtually the same time it learns of the new stream's existence. If the creating endpoint also starts out with zero credit, then it has to wait for the other side to grant some credit before it can start writing data, costing a full roundtip time. Fortunately, as long as both endpoints agree on the same value, there is nothing stopping them from assigning a nonzero amount of starting credit to the creator of a stream. The particular value might be "hardcoded" into a protocol, or it could be set during connection setup. When no other specifics are given however, vanilla "bymux" means starting credit of zero for the creator of each new stream.
...
The second restriction for new ids is that the id may not be in use for another stream. When receiving a packet trying to create a stream with an already active id, the whole connection must be immediately terminated. A stream id is considered active by an endpoint until it has both sent and received `Close` and `StopRead` packets for that stream.

## Initial State

When setting up a bymux connection, the initial state can be arbitrary, as long as both sides agree on the same state. For example a protocol might start out with certain streams already available, possibly even with nonzero payloads, to save a few roundtrips at startup. It might even begin in a state that pretends that global `Close` and `StopRead` packets had already been sent, thus staticaly fixing the set of available streams. The initial state might be "hardcoded" into a protocol, or it could be set during connection setup. When no other specifics are given however, vanilla "bymux" means that initially there are no streams, both endpoints have global credt of zero, and no `Close` and `StopRead` packets have been sent yet.
...
The state for a stream can be released once `Close` and `StopRead` packets have been both sent and received on that stream.
